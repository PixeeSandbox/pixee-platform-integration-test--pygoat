...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if os == 'win':
                command = ['nslookup', domain]  # Validate domain using regex: re.match(r'^[a-zA-Z0-9.-]+$', domain)
            else:
                command = ['dig', domain]  # Validate domain using regex: re.match(r'^[a-zA-Z0-9.-]+$', domain)
            try:
