...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # TODO: Replace the following regex with proper whitelist validation for domain
            if not re.fullmatch(r'^[\w\.\-]+$', domain):
                return HttpResponseBadRequest("Invalid domain")
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command=['nslookup', domain]    # Consider adding additional validation for 'domain'
            else:
                command=['dig', domain]           # Consider adding additional validation for 'domain'
            try:
