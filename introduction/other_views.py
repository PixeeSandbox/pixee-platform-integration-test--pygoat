...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not domain or domain.startswith('-') or not re.fullmatch(r"(?!-)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*", domain):
                output = "Invalid domain"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_FH1eqkf3=['nslookup', domain]
            else:
                command_FH1eqkf3 = ['dig', domain]
            try:
                # output=subprocess.check_output(command_FH1eqkf3,shell=True,encoding="UTF-8")
