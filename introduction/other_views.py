...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command=['nslookup', domain]  # TODO(developer): Replace 'nslookup' with allowed command if needed
            else:
                command = ['dig', domain]  # TODO(developer): Replace 'dig' with allowed command if needed
            try:
