...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # Optional: Validate domain input using regex. Update pattern as necessary.
            if not re.match(r'^[\w\.-]+$', domain):
                return render(request, 'Lab/CMD/cmd_lab.html', {"output": "Invalid domain"})
            os=request.POST.get('os')
            print(os)
            if os=='win':
                command = ['nslookup', domain]  # Validate or sanitize domain if needed
            else:
                command = ['dig', domain]        # Validate or sanitize domain if needed
            try:
