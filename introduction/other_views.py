    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.match(r'^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$', domain):
                # TODO(developer): Replace regex pattern with appropriate allowed domain names if needed
                raise ValueError("Invalid domain")
            # TODO(developer): Replace the placeholder regex pattern with the appropriate domain validation
            if not re.match(r'^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$', domain):
                return HttpResponseBadRequest("Invalid domain format")
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_list = ['nslookup', domain]
            else:
