...
def _is_valid_hostname(domain):
    return re.fullmatch(r"(?:[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?))*", domain) is not None
...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command_8v_0bjLw=['nslookup', domain]
            else:
                command_8v_0bjLw = ['dig', domain]
            try:
                # output=subprocess.check_output(command_8v_0bjLw,shell=True,encoding="UTF-8")
