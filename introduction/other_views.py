...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # Validate that the domain contains only safe characters (letters, digits, underscores, hyphens, and dots)
            if not re.match(r'^[-\w.]+$', domain):
                # TODO: Replace the placeholder error message with the appropriate user feedback
