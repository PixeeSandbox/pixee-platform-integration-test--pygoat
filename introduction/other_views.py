...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.fullmatch(r'(?=.{1,253}$)(?:localhost|(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*|(?:\d{1,3}\.){3}\d{1,3})', domain):
                return render(request,'Lab/CMD/cmd_lab.html',{"output":"Something went wrong"})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_4Fhq9wDJ=["nslookup", domain]
            else:
                command_4Fhq9wDJ = ["dig", domain]
            try:
                # output=subprocess.check_output(command_4Fhq9wDJ,shell=True,encoding="UTF-8")
