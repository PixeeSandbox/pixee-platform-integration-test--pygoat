...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            domain = domain.strip()
            if (":" in domain) or (re.fullmatch(r"[0-9.]+", domain) and "." in domain):
                try:
                    ipaddress.ip_address(domain)
                except ValueError:
                    raise ValueError("Invalid domain")
            elif not re.fullmatch(r"(?=.{1,253}\.?$)(?!-)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*\.?", domain):
                raise ValueError("Invalid domain")
