...
SAFE_HOSTNAME_RE = re.compile(r'^(?=.{1,253}$)(?!-)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*$')


def _is_safe_domain(domain):
    if not domain or any(char.isspace() for char in domain) or domain.startswith('-'):
        return False
    try:
        ipaddress.ip_address(domain)
        return True
    except ValueError:
        return bool(SAFE_HOSTNAME_RE.fullmatch(domain))
