...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.fullmatch(r"(?=.{1,253}$)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63}|localhost", domain):
                return render(request,'Lab/CMD/cmd_lab.html',{"output":"Invalid domain"})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_x2dMe4y7=["nslookup", domain]
            else:
                command_x2dMe4y7 = ["dig", domain]
            try:
                # output=subprocess.check_output(command_x2dMe4y7,shell=True,encoding="UTF-8")
