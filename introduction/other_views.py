...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not domain or not re.fullmatch(r'(?=.{1,253}$)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63}', domain):
                return HttpResponseBadRequest("Invalid domain")
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_efinLLnC=["nslookup", domain]
            else:
                command_efinLLnC = ["dig", domain]
            try:
                # output=subprocess.check_output(command_efinLLnC,shell=True,encoding="UTF-8")
