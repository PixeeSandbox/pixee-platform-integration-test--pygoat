...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # TODO: Validate that 'domain' contains only allowed characters (e.g., valid hostname) to mitigate command injection
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command=["nslookup", domain]  # Validate 'domain' if necessary
            else:
                command = ["dig", domain]  # Validate 'domain' if necessary
            try:
