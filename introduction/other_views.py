...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            domain = (request.POST.get('domain') or '').replace("https://www.", '', 1)
            ...
            if(os=='win'):
                command_MHo9pdgE=["nslookup", domain]
            else:
                command_MHo9pdgE = ["dig", domain]
            try:
                # output=subprocess.check_output(command_MHo9pdgE,shell=True,encoding="UTF-8")
