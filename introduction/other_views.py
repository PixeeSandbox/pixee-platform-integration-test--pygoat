    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.match(r"^[\w\.-]+$", domain):
                return HttpResponse('Invalid domain', status=400)  # TODO(developer): Customize error message and behavior if needed
            ...
            try:
                ...
                process = subprocess.Popen(
                    command,
                    shell=False,  # TODO(developer): Verify if additional arguments are needed for security context
