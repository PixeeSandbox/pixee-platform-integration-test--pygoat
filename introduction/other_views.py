...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # TODO: Validate 'domain' against allowed patterns to prevent command injection
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command = ["nslookup", domain]  # Consider validating 'domain' against allowed patterns
            else:
                command = ["dig", domain]       # Consider validating 'domain' against allowed patterns
            try:
