...
VALID_DOMAIN_RE = re.compile(r'^(?=.{1,253}\.?$)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*\.?$')


def _is_valid_ip_literal(domain):
    try:
...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            domain=domain.strip()
            if not (VALID_DOMAIN_RE.fullmatch(domain) or _is_valid_ip_literal(domain)):
