...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            domain_pattern = re.compile(r'^(?=.{1,253}$)(?:\d{1,3}(?:\.\d{1,3}){3}|(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*\.?)$')
            ...
            print(os)
            if(os=='win'):
                command_VNYTi3Hs=["nslookup", domain]
            else:
                command_VNYTi3Hs = ["dig", domain]
            try:
                # output=subprocess.check_output(command_VNYTi3Hs,shell=True,encoding="UTF-8")
