...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # Allow only hostname/FQDN characters (letters, digits, hyphens, and dots) to prevent shell metacharacters.
            ...
            print(os)
            if(os=='win'):
                command_7N_7p2dR=['nslookup', domain]
            else:
                command_7N_7p2dR = ['dig', domain]
            try:
                # output=subprocess.check_output(command_7N_7p2dR,shell=True,encoding="UTF-8")
