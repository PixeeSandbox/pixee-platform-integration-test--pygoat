...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):  # Replace the regex with an appropriate one for your use-case
                return HttpResponseBadRequest("Invalid domain")
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command = ['nslookup', domain]  # Use a list to avoid shell injection
            else:
                command = ['dig', domain]
            try:
