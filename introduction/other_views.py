    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.match(r'^([a-zA-Z0-9.-]+)$', domain):
                # TODO(developer): Replace "Invalid domain" with your actual error handling
                return render(request, 'Lab/CMD/cmd_lab.html', {"output": "Invalid domain"})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command=["nslookup", domain]  # TODO(developer): Verify domain sanitation is adequate
            else:
                command=["dig", domain]  # TODO(developer): Verify domain sanitation is adequate
            try:
