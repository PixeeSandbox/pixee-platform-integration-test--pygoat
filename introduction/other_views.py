...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if not re.fullmatch(r'(?:\d{1,3}(?:\.\d{1,3}){3}|(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*\.?)', domain):
                return HttpResponseBadRequest("Invalid domain")
            if(os=='win'):
                command_y9ScrwpQ=['nslookup', domain]
            else:
                command_y9ScrwpQ = ['dig', domain]
            try:
                # output=subprocess.check_output(command_y9ScrwpQ,shell=True,encoding="UTF-8")
