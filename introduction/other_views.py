...
SAFE_CMD_LAB3_HOSTNAME = re.compile(
    r'^(?=.{1,253}\Z)(?:'
    r'localhost\.?'
...
def _is_safe_cmd_lab3_domain(domain):
    if not domain:
        return False
    if len(domain) > 253:
        return False
    try:
        ...
        if re.fullmatch(r'[0-9.]+', domain):
            return False
        return SAFE_CMD_LAB3_HOSTNAME.fullmatch(domain) is not None
...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not _is_safe_cmd_lab3_domain(domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
