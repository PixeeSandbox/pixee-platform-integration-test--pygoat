...
DOMAIN_ALLOWLIST_RE = re.compile(
    r'^(?=.{1,253}$)(?:'
...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command_9sjbbruy=["nslookup", domain]
            else:
                command_9sjbbruy = ["dig", domain]
            try:
                # output=subprocess.check_output(command_9sjbbruy,shell=True,encoding="UTF-8")
