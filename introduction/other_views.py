...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.fullmatch(r'(?=.{1,253}$)(?:(?:\d{1,3}\.){3}\d{1,3}|(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*)', domain):
                output = "Invalid domain format"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_WSKVb1KV=["nslookup", domain]
            else:
                command_WSKVb1KV = ["dig", domain]
            try:
                # output=subprocess.check_output(command_WSKVb1KV,shell=True,encoding="UTF-8")
