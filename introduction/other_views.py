...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command = ['nslookup', domain]  # Consider adding domain validation to allow only expected characters
            else:
                command = ['dig', domain]  # Consider adding domain validation to allow only expected characters
            try:
