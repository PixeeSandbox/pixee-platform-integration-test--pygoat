...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command = ['nslookup', domain]  # Developer customization: add further sanitization if needed
            else:
                command = ['dig', domain]  # Developer customization: add further sanitization if needed
            try:
