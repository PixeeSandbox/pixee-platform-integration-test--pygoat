...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if os == 'win':
                command = ['nslookup', domain]  # use list to avoid shell injection
            else:
                command = ['dig', domain]
            try:
                # output=subprocess.check_output(command,shell=True,encoding="UTF-8")
                # TODO: Validate the 'domain' input using an appropriate regex pattern (allow only alphanumeric characters, hyphens, and dots)
