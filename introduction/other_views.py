...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if not re.fullmatch(r'(?:https://www\.)?(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?(?:\.[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)*)', domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            if domain.startswith("https://www."):
                domain = domain.replace("https://www.",'', 1)
            if(os=='win'):
                command_ZSiO_T4z=["nslookup", domain]
            else:
                command_ZSiO_T4z = ["dig", domain]
            try:
                # output=subprocess.check_output(command_ZSiO_T4z,shell=True,encoding="UTF-8")
