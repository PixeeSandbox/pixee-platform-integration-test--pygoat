...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.fullmatch(r'(?i)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9-]{2,63}|(?:\d{1,3}\.){3}\d{1,3}', domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_H3MG7Fna=["nslookup", domain]
            else:
                command_H3MG7Fna = ["dig", domain]
            try:
                # output=subprocess.check_output(command_H3MG7Fna,shell=True,encoding="UTF-8")
