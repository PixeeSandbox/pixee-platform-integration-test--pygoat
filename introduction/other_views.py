...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            domain=(request.POST.get('domain') or '').strip().rstrip('.')
            os=request.POST.get('os')
            print(os)
            validated_domain=None
            try:
                validated_domain=str(ipaddress.ip_address(domain))
            except ValueError:
                if domain and len(domain) <= 253 and re.fullmatch(r'(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*', domain):
