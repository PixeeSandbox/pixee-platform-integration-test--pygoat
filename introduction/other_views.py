...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not domain or not re.fullmatch(r"(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63}|localhost|(?:\d{1,3}\.){3}\d{1,3}|[0-9A-Fa-f:]+", domain):
                output = "Invalid domain"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command__HFGpGXM=["nslookup", domain]
            else:
                command__HFGpGXM = ["dig", domain]
            try:
                # output=subprocess.check_output(command__HFGpGXM,shell=True,encoding="UTF-8")
