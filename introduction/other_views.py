...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            domain=(domain or '').strip().replace("https://www.", '')
            ...
            print(os)
            if(os=='win'):
                command_P4_8kARs=["nslookup", domain]
            else:
                command_P4_8kARs = ["dig", domain]
            try:
                # output=subprocess.check_output(command_P4_8kARs,shell=True,encoding="UTF-8")
