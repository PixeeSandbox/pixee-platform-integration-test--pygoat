    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            try:
                ...
                hostname_pattern = r'^(?=.{1,253}\.?$)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.?)+$'
            ...
            print(os)
            if(os=='win'):
                command_QOOQyAj_=["nslookup", domain]
            else:
                command_QOOQyAj_ = ["dig", domain]
            try:
                # output=subprocess.check_output(command_QOOQyAj_,shell=True,encoding="UTF-8")
