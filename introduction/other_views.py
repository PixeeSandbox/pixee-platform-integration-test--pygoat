...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.fullmatch(r"(?:\d{1,3}(?:\.\d{1,3}){3}|(?=.{1,253}$)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*)", domain) or (
                re.fullmatch(r"\d{1,3}(?:\.\d{1,3}){3}", domain) and not all(0 <= int(octet) <= 255 for octet in domain.split('.'))
            ):
