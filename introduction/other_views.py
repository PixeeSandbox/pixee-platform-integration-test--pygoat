...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.fullmatch(r'(?=.{1,253}$)(?:localhost|(?:(?!-)[A-Za-z0-9-]{1,63}(?<!-))(?:\.(?!-)[A-Za-z0-9-]{1,63}(?<!-))*)', domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_lxOda3uH=['nslookup', domain]
            else:
                command_lxOda3uH = ['dig', domain]
            try:
                # output=subprocess.check_output(command_lxOda3uH,shell=True,encoding="UTF-8")
