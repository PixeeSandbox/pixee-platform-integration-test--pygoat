...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if not (re.fullmatch(r'(?:\d{1,3}\.){3}\d{1,3}', domain) or re.fullmatch(r'(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*\.?', domain)):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            try:
                command_gEgEZUYk = ["nslookup", domain] if (os=='win') else ["dig", domain]
