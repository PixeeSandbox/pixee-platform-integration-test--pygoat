    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            try:
                ...
                if (not domain or domain.startswith('-') or re.search(r'\s|[;&|`$<>\\"\'()]', domain) or not re.fullmatch(r'(?:(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*|(?:\d{1,3}(?:\.\d{1,3}){3})|(?:[A-Fa-f0-9:]+))', domain)):
