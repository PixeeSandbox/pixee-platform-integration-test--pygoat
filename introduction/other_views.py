...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command_zW9gMsRL=["nslookup", domain]
            else:
                command_zW9gMsRL = ["dig", domain]
            try:
                # output=subprocess.check_output(command_zW9gMsRL,shell=True,encoding="UTF-8")
                result = subprocess.run(command_zW9gMsRL, capture_output=True, text=True)
                data = result.stdout
