...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command = ['nslookup', domain]  # TODO(developer): Validate domain input according to your application's security policy
            else:
                command = ['dig', domain]  # TODO(developer): Validate domain input according to your application's security policy
            try:
