...
_HOSTNAME_RE = re.compile(r"(?=.{1,253}\Z)(?:localhost|(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.?)\Z")


def _is_valid_hostname(domain):
    return isinstance(domain, str) and bool(_HOSTNAME_RE.fullmatch(domain))
