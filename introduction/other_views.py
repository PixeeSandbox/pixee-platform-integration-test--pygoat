...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not domain:
                return render(request, 'Lab/CMD/cmd_lab.html', {'output': 'Domain is required.'})
            domain=domain.replace("https://www.",'')
            # Validate domain to avoid constructing OS commands from untrusted input
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):  # Validate domain to prevent command injection by ensuring only allowed characters
