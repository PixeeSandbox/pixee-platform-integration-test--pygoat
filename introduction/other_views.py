    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):
                # TODO(developer): Replace <placeholder> with the actual error handling logic for invalid domain
                return render(request, 'Lab/CMD/cmd_lab.html', {"output": "Invalid domain"})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command=['nslookup', domain]  # TODO(developer): Adjust the command arguments if required
            else:
                command=['dig', domain]
            try:
