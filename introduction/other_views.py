...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # Validate domain input with placeholder regex pattern; replace with appropriate pattern if needed
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):
                return render(request, 'Lab/CMD/cmd_lab.html', {"output": "Invalid domain"})
            os=request.POST.get('os')
            print(os)
            if os == 'win':
                command = ['nslookup', domain]  # Construct command as a list
            else:
                command = ['dig', domain]
            try:
                ...
                process = subprocess.Popen(
                    command,
                    shell=False,  # Changed to False to avoid shell injection
