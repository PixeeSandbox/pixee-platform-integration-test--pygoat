...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            domain=request.POST.get('domain', '').replace("https://www.",'').replace("http://www.", '').replace("https://", '').replace("http://", '').strip()
            ...
            print(os)
            if(os=='win'):
                command_5NUO2VPw=["nslookup", domain]
            else:
                command_5NUO2VPw = ["dig", domain]
            try:
                # output=subprocess.check_output(command_5NUO2VPw,shell=True,encoding="UTF-8")
