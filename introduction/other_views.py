...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command=["nslookup", domain]  # Validate that 'domain' input is rigorously validated or sanitized before use
            else:
                command = ["dig", domain]  # Validate that 'domain' input is rigorously validated or sanitized before use
            try:
