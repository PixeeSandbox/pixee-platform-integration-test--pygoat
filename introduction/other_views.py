...
def _is_valid_hostname(hostname):
    return bool(hostname and not SHELL_METACHAR_RE.search(hostname) and HOSTNAME_RE.fullmatch(hostname))
    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            try:
                ...
                if(os=='win'):
                    command_UGg3QOmA=['nslookup', domain]
                else:
                    command_UGg3QOmA = ['dig', domain]
                # output=subprocess.check_output(command_UGg3QOmA,shell=True,encoding="UTF-8")
