...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            domain=domain.replace("https://www.",'').replace("http://www.",'').replace("www.", '')
            ...
            print(os)
            if(os=='win'):
                command_WghXGLGZ=["nslookup", domain]
            else:
                command_WghXGLGZ = ["dig", domain]
            try:
                # output=subprocess.check_output(command_WghXGLGZ,shell=True,encoding="UTF-8")
