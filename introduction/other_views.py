...
def _is_valid_domain(domain):
    ...
    return re.fullmatch(r"(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*", hostname) is not None
...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if(os=='win'):
                command_ZlCfZGaP=["nslookup", domain]
            else:
                command_ZlCfZGaP = ["dig", domain]
            try:
                # output=subprocess.check_output(command_ZlCfZGaP,shell=True,encoding="UTF-8")
