...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            domain_pattern = re.compile(r"(?=.{1,253}\Z)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z0-9]{2,63}\.?\Z")
            if not domain or not domain_pattern.fullmatch(domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            if(os=='win'):
                command_RKykpAqg=["nslookup", domain]
            else:
                command_RKykpAqg = ["dig", domain]
            try:
                # output=subprocess.check_output(command_RKykpAqg,shell=True,encoding="UTF-8")
