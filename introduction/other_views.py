    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if (len(domain) > 253 or not re.fullmatch(r"[A-Za-z0-9.-]+", domain)
                    ...
                    or '..' in domain
                    or any(not re.fullmatch(r"[A-Za-z0-9-]{1,63}", label) or label.startswith('-') or label.endswith('-')
            ...
            print(os)
            if(os=='win'):
                command_9s_IZV9j=['nslookup', domain]
            else:
                command_9s_IZV9j = ['dig', domain]
            try:
                # output=subprocess.check_output(command_9s_IZV9j,shell=True,encoding="UTF-8")
