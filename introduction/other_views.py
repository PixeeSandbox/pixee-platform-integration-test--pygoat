...
DOMAIN_RE = re.compile(r'^(?=.{1,253}$)(?:localhost|(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63})$')
...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command_Dh4hDNWx=["nslookup", domain]
            else:
                command_Dh4hDNWx = ["dig", domain]
            try:
                # output=subprocess.check_output(command_Dh4hDNWx,shell=True,encoding="UTF-8")
