...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # Validate domain to allow only alphanumerics, dashes, and dots
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):
                # TODO: Replace with appropriate validation logic
                return render(request, 'Lab/CMD/cmd_lab.html', {"output":"Invalid domain"})
            os=request.POST.get('os')
            print(os)
            if os == 'win':
                command = ["nslookup", domain]  # Construct command as list
            else:
                command = ["dig", domain]
            try:
                ...
                process = subprocess.Popen(
                    command,
                    shell=False,    # Remove or set shell=False
