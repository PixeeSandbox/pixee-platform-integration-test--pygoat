...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # TODO: Validate the 'domain' variable (e.g., using regex to allow only valid domain characters)
            # Example: if not re.match('^[a-zA-Z0-9.-]+$', domain): raise ValueError("Invalid domain")
            os=request.POST.get('os')
            logging.debug('Operating System: %s', os)
            if os == 'win':
