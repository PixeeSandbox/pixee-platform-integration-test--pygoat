...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # Validate domain: allow only alphanumeric characters, dot, and hyphen (replace placeholder as necessary)
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):
                raise ValueError("Invalid domain input")
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command=["nslookup", domain]  # Consider validating 'domain' for allowed characters
            else:
                command = ["dig", domain]
            try:
