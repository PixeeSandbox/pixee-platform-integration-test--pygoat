...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if not domain or not re.fullmatch(r"(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63}", domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            if(os=='win'):
                command_3Y2SO68K=["nslookup", domain]
            else:
                command_3Y2SO68K = ["dig", domain]
            try:
                # output=subprocess.check_output(command_3Y2SO68K,shell=True,encoding="UTF-8")
