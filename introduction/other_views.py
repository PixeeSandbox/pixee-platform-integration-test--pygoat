...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if not re.fullmatch(r"(?:localhost|(?:(?:\d{1,3}\.){3}\d{1,3})|(?=.{1,253}$)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63})", domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            if(os=='win'):
                command_YGojkMkf=['nslookup', domain]
            else:
                command_YGojkMkf = ['dig', domain]
            try:
                # output=subprocess.check_output(command_YGojkMkf,shell=True,encoding="UTF-8")
