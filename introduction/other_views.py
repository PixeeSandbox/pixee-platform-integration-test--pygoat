...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # Validate the 'domain' input (e.g. using a regex pattern)
            if not re.match(r'^[A-Za-z0-9.-]+$', domain):
                return HttpResponseBadRequest("Invalid domain format.")
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command = ["nslookup", domain]  # Consider validating 'domain' before use
            else:
                command = ["dig", domain]  # Consider validating 'domain' before use
            try:
