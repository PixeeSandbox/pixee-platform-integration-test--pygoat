...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            domain=domain.replace("https://www.",'') if domain else domain
            os=request.POST.get('os')
            print(os)
            hostname_pattern = re.compile(r'^(?=.{1,253}\Z)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?(?:\.[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)*\.?$)')
            if not domain or not hostname_pattern.fullmatch(domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            if(os=='win'):
                command_etzFkTEZ=['nslookup', domain]
            else:
                command_etzFkTEZ = ['dig', domain]
            try:
                # output=subprocess.check_output(command_etzFkTEZ,shell=True,encoding="UTF-8")
