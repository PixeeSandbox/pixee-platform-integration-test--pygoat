...
HOSTNAME_RE = re.compile(r"(?=.{1,253}\Z)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*\.?\Z")
...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command_jiJPpJ58=["nslookup", domain]
            else:
                command_jiJPpJ58 = ["dig", domain]
            try:
                # output=subprocess.check_output(command_jiJPpJ58,shell=True,encoding="UTF-8")
