...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not domain or not re.fullmatch(r'(https://www\.)?[A-Za-z0-9.-]+', domain):
            ...
            print(os)
            if(os=='win'):
                command_CyAmvD3k=['nslookup', domain]
            else:
                command_CyAmvD3k = ['dig', domain]
            try:
                # output=subprocess.check_output(command_CyAmvD3k,shell=True,encoding="UTF-8")
