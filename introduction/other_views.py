...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command = ["nslookup", domain]    # Consider validating 'domain' to ensure no malicious input
            else:
                command = ["dig", domain]         # Consider validating 'domain' to ensure no malicious input
            try:
