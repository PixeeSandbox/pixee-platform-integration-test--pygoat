...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):  # TODO(developer): Replace <placeholder> with the appropriate allowed pattern for domain
                return render(request, 'Lab/CMD/cmd_lab.html', {"output": "Invalid domain format"})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command=['nslookup', domain]  # TODO(developer): Validate allowed command arguments
            else:
                command=['dig', domain]  # TODO(developer): Validate allowed command arguments
            try:
