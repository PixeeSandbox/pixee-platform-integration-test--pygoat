...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not domain or re.search(r"\s", domain) or not re.fullmatch(r"[A-Za-z0-9.-]+", domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_8BNPG6pF=['nslookup', domain]
            else:
                command_8BNPG6pF = ['dig', domain]
            try:
                # output=subprocess.check_output(command_8BNPG6pF,shell=True,encoding="UTF-8")
