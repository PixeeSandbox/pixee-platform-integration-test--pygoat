...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            # Validate domain: allow only alphanumeric characters, dots, and hyphens to prevent command injection
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):
                return HttpResponseBadRequest('Invalid domain')
            os=request.POST.get('os')
            print(os)
            # Construct command as list to avoid shell injection vulnerabilities
            if(os=='win'):
