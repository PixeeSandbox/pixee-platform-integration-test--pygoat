...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            # TODO: Validate 'domain' input against allowed characters
            if(os=='win'):
                command=["nslookup", domain]  # TODO: validate 'domain' input further if necessary
            else:
                command = ["dig", domain]  # TODO: validate 'domain' input further if necessary
            try:
