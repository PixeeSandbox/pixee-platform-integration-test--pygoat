...
def _is_valid_hostname(domain):
    return bool(domain and _HOSTNAME_RE.fullmatch(domain))
...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            if(os=='win'):
                command_ZVxi4szB=["nslookup", domain]
            else:
                command_ZVxi4szB = ["dig", domain]
            try:
                # output=subprocess.check_output(command_ZVxi4szB,shell=True,encoding="UTF-8")
