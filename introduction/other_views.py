...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.fullmatch(r"(?:localhost|(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63})", domain):
                output = "Invalid domain"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_rqfV6e90=["nslookup", domain]
            else:
                command_rqfV6e90 = ["dig", domain]
            try:
                # output=subprocess.check_output(command_rqfV6e90,shell=True,encoding="UTF-8")
