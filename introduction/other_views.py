...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            print(os)
            normalized_domain = domain[:-1] if domain and domain.endswith('.') else domain
            ...
            try:
                ...
                if not re.fullmatch(r'(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*', normalized_domain):
                    output = "Invalid domain"
                    return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
