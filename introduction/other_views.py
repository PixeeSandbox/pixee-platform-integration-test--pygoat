...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not domain or re.fullmatch(r'(?:https://www\.)?(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63}', domain) is None:
            ...
            print(os)
            if(os=='win'):
                command_gNf0NcA7=["nslookup", domain]
            else:
                command_gNf0NcA7 = ["dig", domain]
            try:
                # output=subprocess.check_output(command_gNf0NcA7,shell=True,encoding="UTF-8")
