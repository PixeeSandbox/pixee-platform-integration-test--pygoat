...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            domain = domain.lower()
            if not re.fullmatch(r'(?=.{1,253}$)(?=.{1,63}(?:\.|$))(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63}', domain):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_Dvy8m627=['nslookup', domain]
            else:
                command_Dvy8m627 = ['dig', domain]
            try:
                # output=subprocess.check_output(command_Dvy8m627,shell=True,encoding="UTF-8")
