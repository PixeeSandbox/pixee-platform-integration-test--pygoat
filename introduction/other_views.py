...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            domain=domain.strip().lower().replace("https://www.",'').replace("http://www.",'').replace("www.",'')
            os=request.POST.get('os')
            print(os)
            if (not domain or domain.startswith('-') or not re.fullmatch(r'(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63}', domain)):
