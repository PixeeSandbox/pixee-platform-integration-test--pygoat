...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not re.fullmatch(r"(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*\.?|(?:\d{1,3}(?:\.\d{1,3}){3})", domain):
                return render(request,'Lab/CMD/cmd_lab.html',{"output":"Something went wrong"})
            os=request.POST.get('os')
            print(os)
            try:
                if(os=='win'):
                    process = subprocess.run(['nslookup', domain], shell=False, capture_output=True, text=True)
                else:
                    process = subprocess.run(['dig', domain], shell=False, capture_output=True, text=True)
                data = process.stdout
