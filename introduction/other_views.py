...
def cmd_lab3(request):
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            if not domain or not re.fullmatch(r'[A-Za-z0-9.-]+', domain) or domain.startswith('-'):
                output = "Something went wrong"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            os=request.POST.get('os')
            print(os)
            if(os=='win'):
                command_sZJxiWIj=["nslookup", domain]
            else:
                command_sZJxiWIj = ["dig", domain]
            try:
                # output=subprocess.check_output(command_sZJxiWIj,shell=True,encoding="UTF-8")
