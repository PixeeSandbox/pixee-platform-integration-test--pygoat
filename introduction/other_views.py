    ...
    if request.user.is_authenticated:
        if (request.method=="POST"):
            ...
            try:
                ...
                domain_is_valid = hostname_re is not None
            if not domain_is_valid:
                output = "Invalid domain"
                return render(request,'Lab/CMD/cmd_lab.html',{"output":output})
            if(os=='win'):
                command_Mkz_GQSa=["nslookup", domain]
            else:
                command_Mkz_GQSa = ["dig", domain]
            try:
                # output=subprocess.check_output(command_Mkz_GQSa,shell=True,encoding="UTF-8")
