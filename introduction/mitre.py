...
def _is_valid_host_token(value):
    if not value:
        return False
    try:
        ...
        host_pattern = re.compile(r"(?=.{1,253}\Z)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*")
...
def mitre_lab_17_api(request):
    if request.method == "POST":
        ...
        if not _is_valid_host_token(ip):
            return JsonResponse({'error': 'Invalid IP/host'}, status=400)
        command = ['nmap', ip]
